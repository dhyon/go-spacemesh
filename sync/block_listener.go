...
func (bl *BlockListener) HandleNewBlock(blk *types.Block) bool {
	...
	bl.Log.With().Info("got new block", log.Uint64("id", uint64(blk.Id)), log.Int("txs", len(blk.TxIds)), log.Int("atxs", len(blk.AtxIds)), log.Int("epoch", int(bl.currentLayer.GetEpoch(bl.layersPerEpoch))))
	//check if known
